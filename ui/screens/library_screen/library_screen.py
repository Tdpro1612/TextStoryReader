...
class LibraryScreen(Screen):
    ...
    def update_library_view(self):
        ...
        if 'grid_layout_stories' in self.ids:
            ...
            if hasattr(app, 'managers') and hasattr(app.managers, 'book_manager'):
                ...
                self.show_error_popup("Lỗi Khởi Tạo", "Không thể tìm thấy BookManager.\nVui lòng kiểm tra cấu hình ứng dụng.")
                return
            ...
            self.show_error_popup("Lỗi Giao Diện", "Không thể tìm thấy thành phần giao diện 'grid_layout_stories'.\nVui lòng kiểm tra file KV.")
    ...
    def open_book(self, instance):
        ...
        if hasattr(app, 'managers') and hasattr(app.managers, 'book_manager'):
            ...
            print("ERROR (LibraryScreen): BookManager not found when trying to open book.")
            self.show_error_popup("Lỗi Hệ Thống", "Không thể truy cập BookManager để mở sách.")
            return

        # Sử dụng BookManager để đọc nội dung sách
        book_data, chapter_list, error_type = book_manager_instance.read_book_content(book_filename)
        book_name = os.path.splitext(os.path.basename(book_filename))[0].replace('_', ' ').title() # Lấy tên file base
        ...
        if error_type == "file_not_found":
            ...
            print(f"DEBUG (open_book): Không thể đọc nội dung sách: {book_name} ({book_filename})")
            self.show_error_popup(
                "Lỗi Đọc Sách",
        ...
        if book_data:
            ...
            if app.root.has_screen('reader_screen') and app.root.has_screen('chapter_screen'):
                ...
                print(f"DEBUG (open_book): Sách '{book_name}' đã được tải và chuyển sang màn hình đọc.")
            else:
                print("ERROR (open_book): ReaderScreen or ChapterScreen not found in ScreenManager.")
                self.show_error_popup("Lỗi Điều Hướng", "Không thể tìm thấy màn hình đọc hoặc màn hình chương.\nVui lòng kiểm tra cấu hình ScreenManager.")
        else:
            print(f"DEBUG (open_book): Không có dữ liệu sách sau khi đọc: {book_name} ({book_filename})")
            self.show_error_popup(
                "Lỗi Đọc Sách",
    ...
    def show_error_popup(self, title, message):
        ...
        content.add_widget(Label(text=message, halign='center', valign='middle', text_size=(dp(250), None)))
