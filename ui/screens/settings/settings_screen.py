...
class SettingsScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.font_list = []
        print("DEBUG (SettingsScreen): Init SettingsScreen, font_list initialized:", self.font_list)
    ...
    def set_app_settings(self, settings_instance):
        ...
        print("DEBUG (SettingsScreen): AppSettings instance đã được gán cho SettingsScreen.")
    ...
    def get_app_settings(self):
        ...
        if self._app_settings_instance is None:
            ...
            print("CẢNH BÁO (SettingsScreen): _app_settings_instance chưa được gán. Thử lấy từ App.get_running_app().")
    ...
    def on_pre_enter(self):
        ...
        if app_settings is None:
            print("LỖI (SettingsScreen): Không thể tải cài đặt UI vì app_settings chưa có.")
            return

        # Load danh sách font
        font_dir = "assets/fonts"
        if os.path.exists(font_dir):
            self.font_list = [f for f in os.listdir(font_dir) if f.lower().endswith(('.ttf', '.otf'))]
        else:
            self.font_list = []
        ...
        if 'font_spinner' in self.ids:
            ...
            current_font_name = app_settings.get_setting('font_name', 'Roboto') # Lấy từ settings
            if current_font_name in self.font_list:
                self.ids.font_spinner.text = current_font_name
            elif self.font_list: # Nếu font đã lưu không có, chọn font đầu tiên trong danh sách
        ...
        if 'font_size_spinner' in self.ids:
            ...
            self.ids.font_size_spinner.text = font_size_map.get(settings.get('font_size', 20), 'Vừa')
    ...
    def save_settings(self):
        ...
        app_settings = self.get_app_settings()
        if app_settings is None:
            print("LỖI (SettingsScreen): Không thể lưu cài đặt vì app_settings chưa có.")
            return

        font_size_map_reverse = {
            'Nhỏ': 10,
            'Vừa': 20,
            'Lớn': 30
        }
    ...
    def reset_settings(self):
        ...
        app_settings = self.get_app_settings()
        if app_settings is None:
            print("LỖI (SettingsScreen): Không thể reset cài đặt vì app_settings chưa có.")
            return
