...
class ReaderScreen(Screen):
    ...
    # Dữ liệu của chapter
    current_chapter_index = NumericProperty(
        0
    )  # chỉ số của chapter đang đọc trong danh sách chapter
    chapter_pages = ListProperty([])  # tổng số trang của chapter sau khi chia
    current_reading_page_index = NumericProperty(
        0
    )  # vị trí trang đang đọc được chia trong chapter
    ...
    def on_enter(self, *args):
        print(
            "DEBUG (ReaderScreen): on_enter đã được gọi. Đang tải và áp dụng cài đặt UI mới nhất."
        )
    ...
    def load_and_apply_ui_settings(self):
        ...
        self.current_text_color = get_color_from_hex(
            app_settings.get_setting("text_color")
        )
        self.current_bg_color = get_color_from_hex(
            app_settings.get_setting("background_color")
        )

        print(
            f"DEBUG (ReaderScreen): Cài đặt UI đã tải và áp dụng: "
    ...
    def load_book_content(
        ...
        """
        print(
            f"DEBUG (ReaderScreen): load_book_content được gọi cho '{book_name}' ({book_filename})"
        )
        ...
        print(
            f"DEBUG (ReaderScreen): Thiết lập vị trí mặc định: Chương {self.current_chapter_index}, Trang {self.current_reading_page_index}"
        )
    ...
    def _setup_size_binding(self, instance, value):
        ...
        if "book_content_label" in self.ids:
            self.ids.book_content_label.bind(
                size=self.on_content_display_label_size_changed
            )
            print(
                "DEBUG (ReaderScreen): Đã thiết lập binding kích thước cho book_content_label."
            )
        else:
            print(
                "CẢNH BÁO: Không tìm thấy ID 'book_content_label'. Không thể thiết lập binding kích thước."
            )
    ...
    def on_content_display_label_size_changed(self, instance, value):
        ...
        if new_width != self._last_content_width:
            print(
                f"DEBUG (ReaderScreen): Chiều rộng book_content_label thay đổi: {new_width}. Đang lên lịch phân trang lại."
            )
    ...
    def repaginate_on_next_frame(self, dt):
        print(
            "DEBUG (ReaderScreen): Gọi repaginate_current_chapter từ repaginate_on_next_frame."
        )
    ...
    def repaginate_current_chapter(self):
        ...
        if not self.book_data or not (
            0 <= self.current_chapter_index < len(self.book_data)
        ):
            self.chapter_pages = []
            self.current_reading_page_index = 0
            print(
                "CẢNH BÁO (ReaderScreen): Không có dữ liệu chương để phân trang (chế độ 1 chương/1 trang)."
            )
        ...
        print(
            f"DEBUG (ReaderScreen): Đã 'phân trang' chương {self.current_chapter_index} thành 1 trang."
        )
    ...
    def update_page_content(self):
        print(
            f"DEBUG (ReaderScreen): update_page_content được gọi. Chương: {self.current_chapter_index}, Trang: {self.current_reading_page_index}"
        )

        # Kiểm tra Label hiển thị nội dung (ID KHỚP VỚI KV)
        if "book_content_label" not in self.ids:
            print(
                "LỖI (ReaderScreen): Không tìm thấy widget 'book_content_label' để cập nhật nội dung."
            )
            return
        ...
        if self.chapter_pages:
            ...
            print(
                f"DEBUG (ReaderScreen): Đã đặt nội dung sách (một phần): {content[:100]}..."
            )
            ...
            if "book_content_scroll_view" in self.ids:
                ...
                print(
                    "CẢNH BÁO (ReaderScreen): Không tìm thấy 'book_content_scroll_view' để cuộn."
                )
    ...
    def jump_to_chapter(self, target_chapter_index):
        ...
        """
        print(
            f"DEBUG (ReaderScreen): Yêu cầu nhảy đến chương có Index: {target_chapter_index}"
        )
        ...
        if not (0 <= target_chapter_index < len(self.book_data)):
            print(
                f"CẢNH BÁO (ReaderScreen): Chỉ số chương {target_chapter_index} nằm ngoài giới hạn "
        ...
        print(
            f"DEBUG (ReaderScreen): Đã đặt chương hiện tại thành {self.current_chapter_index}, trang {self.current_reading_page_index}."
        )
