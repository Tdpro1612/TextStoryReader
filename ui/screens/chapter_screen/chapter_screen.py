...
class ChapterScreen(Screen):
    ...
    def load_chapters(self, chapter_data_list):
        ...
        if not chapter_data_list: # Nếu danh sách chương rỗng
            ...
            for chapter in self.chapters_display_list:
                ...
                item.bind(on_release=lambda x, chap=chapter: self.on_chapter_selected(chap))
                self.chapter_list_container.add_widget(item)
            print(f"DEBUG (ChapterScreen): Đã tải {len(self.chapters_display_list)} chương vào danh sách.")
    ...
    def _go_back(self):
        ...
        if self.manager and self.previous_screen_name:
            print(f"DEBUG (Chapterscreen): Quay lại màn hình: {self.previous_screen_name}")
            ...
            print("CẢNH BÁO (Chapterscreen): Không tìm thấy màn hình trước đó. Quay về màn hình mặc định 'reader_screen'.")
            self.manager.current = 'reader_screen'