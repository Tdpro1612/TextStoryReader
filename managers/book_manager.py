...
# SUPPORTED_EXTENSIONS được định nghĩa toàn cục và duy nhất
SUPPORTED_EXTENSIONS = ['.txt', '.html'] # Giữ đúng với khả năng parser hiện tại, nếu có epub/pdf thì thêm vào
...
if platform == 'android':
    ...
    try:
        ...
        # Thêm các lớp Java cần thiết cho Intent.EXTRA_MIME_TYPES
        # Nếu chưa có, đảm bảo các import này có sẵn
        # Ví dụ: autoclass('android.content.ContentResolver')
        # autoclass('android.provider.OpenableColumns')
        # Hoặc các lớp liên quan đến mảng Java
        # Arrays = autoclass('java.util.Arrays')
        # StringArray = autoclass('java.lang.String[]') # Để tạo mảng String Java
        ...
        class AndroidIntentHandler(PythonJavaClass):
            ...
            def onActivityResult(self, requestCode, resultCode, intent):
                ...
                if resultCode == RESULT_OK and intent is not None:
                    ...
                    self.callback(requestCode, resultCode, None) # Trường hợp khác, truyền None
...
# --- GLOBAL CONSTANTS (ví dụ) ---
REQUEST_CODE_PICK_FILE = 1001
REQUEST_CODE_PICK_FOLDER = 1002

class BookManager(EventDispatcher): # Kế thừa EventDispatcher nếu bạn muốn sử dụng bind/dispatch
    def __init__(self, **kwargs):
        ...
        if platform == 'android' and PythonActivity: # Kiểm tra cả platform và PythonActivity có tồn tại không
            self.is_android = True
            print("DEBUG (BookManager): Đang chạy trên Android, thiết lập đường dẫn Android.")
            self._get_android_paths()

        print(f"DEBUG (BookManager): Thư mục sách (self.book_folder_path): {self.book_folder_path}")
    ...
    def _report_status(self, message):
        ...
        if self.status_callback:
            ...
            print(f"BookManager Status: {message}")  # Fallback nếu callback chưa được thiết lập
    ...
    def get_book_list(self):
        ...
        if not os.path.exists(self.book_folder_path):
            ...
            print(f"DEBUG (BookManager): Thư mục '{self.book_folder_path}' không tồn tại, đã tạo.")
            return []

        files_in_folder = os.listdir(self.book_folder_path)
        print(f"DEBUG (BookManager): Các file được tìm thấy trong '{self.book_folder_path}': {files_in_folder}")

        for filename in files_in_folder:
            for ext in SUPPORTED_EXTENSIONS:  # SỬ DỤNG SUPPORTED_EXTENSIONS TOÀN CỤC
                if filename.lower().endswith(ext): # Đảm bảo so sánh không phân biệt chữ hoa/thường
    ...
    def read_book_content(self, book_filename):
        ...
        try:
            if book_filename.lower().endswith('.txt'):
                ...
                print(f"DEBUG (BookManager): Sách TXT '{book_name}' đã được đọc từ '{full_path}'.")
            elif book_filename.lower().endswith('.html'):
                book_data, chapter_list = read_book_html(full_path)
                print(f"DEBUG (BookManager): Sách HTML '{book_name}' đã được đọc từ '{full_path}'.")
            else:
                print(f"DEBUG (BookManager): Định dạng file không được hỗ trợ: {book_filename}")
                return [], [], "unsupported_format"
        except Exception as e:
            print(f"DEBUG (BookManager): Lỗi khi đọc nội dung sách '{full_path}': {e}")
            return [], [], "read_error"

        if not book_data and not chapter_list:  # Kiểm tra cả hai để chắc chắn là có nội dung
            print(f"DEBUG (BookManager): Không có dữ liệu sách sau khi đọc từ '{full_path}'.")
            return [], [], "read_error"
