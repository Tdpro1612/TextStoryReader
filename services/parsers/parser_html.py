...
def read_book_html(full_filepath):
    ...
    try:
        with open(full_filepath, 'r', encoding='utf-8') as f:
            html_content = f.read()
        filename = os.path.splitext(os.path.basename(full_filepath))[0].replace('_', ' ').title()
        ...
        if not body_tag:
            print(f"CẢNH BÁO (read_book_html): Không tìm thấy thẻ <body> trong file '{filename}'.")
            book_data.append({
                'index': 0,
        ...
        current_chapter_title_text = os.path.splitext(filename)[0].replace('_', ' ').title() # Tiêu đề mặc định cho chương đầu tiên/giới thiệu
        chapter_counter = 0
        ...
        for element in body_tag.children:
            ...
            if element.name in main_heading_tags and isinstance(element, Tag):
                ...
                if current_chapter_elements:
                    ...
                    plain_text_content = chapter_content_soup.get_text(separator='\n\n', strip=True)
                ...
                current_chapter_elements = [element] # Thêm thẻ tiêu đề vào nội dung chương mới
        ...
        if current_chapter_elements:
            ...
            for el in current_chapter_elements:
                chapter_content_soup.append(el)
            plain_text_content = chapter_content_soup.get_text(separator=' ', strip=True)
            ...
            if not found_first_main_chapter and not book_data and plain_text_content:
                final_chapter_title = os.path.splitext(filename)[0].replace('_', ' ').title()
        ...
        if not book_data:
            ...
            print(f"DEBUG (read_book_html): File HTML rỗng hoặc không có nội dung hợp lệ.")

        print(f"DEBUG (read_book_html): Đã parse '{filename}' thành {len(book_data)} chương.")
        return book_data, chapter_list_for_chapter_screen
